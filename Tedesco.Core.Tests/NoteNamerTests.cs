...
namespace Tedesco.Tests
{
	public class NoteNamerTests
	{
		...
		{
			Assert.Equal(new Note(0), NoteNamer.PitchOf("C-2"));
		}

		[Fact]
		public void Recognizes_CMinus1_As_Midi_C_12()
		{
			Assert.Equal(new Note(12), NoteNamer.PitchOf("C-1"));
		}

		[Fact]
		public void Recognizes_C3_As_Standard_Middle_C()
		{
			Assert.Equal(new Note(60), NoteNamer.PitchOf("C3"));
		}

		[Fact]
		public void Recognizes_C4_As_Higher_Middle_C()
		{
			Assert.Equal(new Note(60), NoteNamer.PitchOf("C4", MidiOctaveFormat.Higher));
		}

		[Fact]
		public void Recognizes_C1_As_Next_Octave_Midi_Number()
		{
			Assert.Equal(new Note(36), NoteNamer.PitchOf("C1"));
		}
		
		[Fact]
		public void Recognizes_Sharp_Modifiers()
		{
			Assert.Equal(new Note(25), NoteNamer.PitchOf("C#0"));
			Assert.Equal(new Note(37), NoteNamer.PitchOf("C#1"));
		}

		[Fact]
		public void Recognizes_Flat_Modifiers()
		{
			Assert.Equal(new Note(25), NoteNamer.PitchOf("Db0"));
		}
		...
		{
			Assert.Equal(new Note(127), NoteNamer.PitchOf("G8"));
		}
	}
}
